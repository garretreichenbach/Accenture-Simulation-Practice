...
public class ProductGenerator {
	...
	private static final String[] RANDOM_DESC = {
	...
	public Collection<ProductItem> generateProducts() throws IOException {
		...
		for(int i = 0; i < 100; i++) {
			int idx1 = random.nextInt(productImages.length);
			int idx2 = random.nextInt(productImages.length);
			Resource tmp = productImages[idx1];
			productImages[idx1] = productImages[idx2];
			productImages[idx2] = tmp;
		}

		List<ProductItem> products = new ArrayList<>();
		for(Resource image : productImages) {
	...
	private String generateName() {
		return String.format(
				"%s %s",
				RANDOM_ADJECTIVES[random.nextInt(RANDOM_ADJECTIVES.length)],
				RANDOM_NOUNS[random.nextInt(RANDOM_NOUNS.length)]
		);
	}

	private String generateDescription() {
		return RANDOM_DESC[random.nextInt(RANDOM_DESC.length)];
	}
	...
	private Integer generateCost() {
		return (100 * random.nextInt(1000)) + 99;
	}
