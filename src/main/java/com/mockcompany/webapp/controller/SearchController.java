	...
	public Collection<ProductItem> search(@RequestParam("query") String query) {
		...
		for(ProductItem item : allItems) {
			...
			if(query != null && !query.isEmpty()) {
				//If quotes are used, the search should be exact
				if(query.startsWith("\"") && query.endsWith("\"")) matchesSearch = item.getName().toLowerCase().trim().equals(query.toLowerCase().trim().replace("\"", "")) || item.getDescription().toLowerCase().trim().equals(query.toLowerCase().trim().replace("\"", ""));
				else matchesSearch = item.getName().toLowerCase().trim().contains(query.toLowerCase().trim()) || item.getDescription().toLowerCase().trim().contains(query.toLowerCase().trim());
			}
